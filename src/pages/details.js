...
export default class details extends Component{
    ...
    async componentDidMount() {
            ...
            await axios.get('https://petrapi.herokuapp.com/emergence/onereport/'+param, 
            {headers: {
                Authorization: `Bearer ${token}`
            }, timeout:60000
            }).then(response => {
                this.setState({report: response.data.data, isloading: false, iserror: false})
        }).catch(error => {
            this.setState({error: true, isloading: false, iserror: true})
        })
